...
```{r}
# coords land_degradation  land_degradation[,16:17]
# data land_degradation[,0:15]
...
# cluster_raster <- rast(rasterFromXYZ(data.frame(as.data.frame(df_grid_clusters[c('X_proj','Y_proj','clusters')])),
#                                   crs = 32720)) %>%
#   project("EPSG:4326")
...
rf_grid_result <- tune_race_anova(
  ...
  grid = 25  # Try 50 different hyperparameter combinations
)
...
```{r}
library(finetune)  # needed for tune_bayes()

# Define parameter ranges explicitly (optional)
...
# Use tune_bayes instead of tune_race_anova
...
```{r}
# 3. Extraer la importancia de variables
vip_df <- final_fit %>%
  extract_fit_parsnip() %>%
  vip::vi()  # Usa vip::vi() del paquete {vip}

# 4. Visualizarla
var_importance <- vip_df %>%
  mutate(Variable = fct_reorder(Variable, Importance)) %>%  # <- clave para ordenar
